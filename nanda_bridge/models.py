...
class NandaEndpoints(BaseModel):
    ...
    adaptive_resolver: NandaAdaptiveResolver | None = Field(
        None, 
        description="Adaptive resolver for dynamic capability-based routing"
    )
...
class NandaAuthentication(BaseModel):
    ...
    methods: list[str] = Field(
        default_factory=lambda: ["did-auth"],
        description="Supported authentication methods"
    )
    requiredScopes: list[str] | None = Field(
        None,
        description="Required OAuth scopes if using oauth2"
    )
...
class NandaCapabilities(BaseModel):
    ...
    authentication: NandaAuthentication = Field(
        default_factory=NandaAuthentication,
...
class NandaSkill(BaseModel):
    ...
    inputModes: list[str] = Field(default_factory=lambda: ["text"], description="Accepted input types")
    outputModes: list[str] = Field(default_factory=lambda: ["text"], description="Produced output types")
...
class NandaCertification(BaseModel):
    ...
    """
    level: str = Field(..., description="Certification level: 'self-declared', 'verified', 'audited'")
...
class NandaAgentFacts(BaseModel):
    ...
    the x_<registry_name> convention (e.g., x_lyra_space, x_acme).
    """
    # Identity
    id: str = Field(..., description="Agent's unique identifier (UUID or DID)")
    handle: str | None = Field(None, description="NANDA handle (e.g., '@registry:namespace/agent-name')")
    ...
    # Provider
    provider: NandaProvider = Field(..., description="Provider information")
    
    # Connectivity
    endpoints: NandaEndpoints = Field(..., description="Agent endpoints")
    
    # Capabilities
    capabilities: NandaCapabilities = Field(..., description="Agent capabilities")
    skills: list[NandaSkill] = Field(default_factory=list, description="Detailed skill definitions")
    
    # Trust & Verification (production NANDA fields)
    certification: NandaCertification | None = Field(None, description="Trust certification")
    evaluations: NandaEvaluations | None = Field(None, description="Performance evaluations")
    telemetry: NandaTelemetry | None = Field(None, description="Observability configuration")
    
    # Extensions
    metadata: dict[str, Any] = Field(
        default_factory=dict,
        description="Registry-specific extensions (use x_<registry> prefix)"
    )
    
    # Optional proof / attestation
    proof: dict[str, Any] | None = Field(
        None,
        description="Optional attestation payload (e.g., hash/signature metadata)"
    )
    ...
    def create_handle(cls, registry: str, namespace: str, agent_id: str) -> str:
        ...
        Args:
            registry: Registry identifier (e.g., "lyra")
            namespace: Agent namespace (e.g., "space")
            agent_id: Agent identifier (e.g., "collision-avoidance")
            
        Returns:
            NANDA handle (e.g., "@lyra:space/collision-avoidance")
        """
        return f"@{registry}:{namespace}/{agent_id}"


class NandaAgentFactsIndexResponse(BaseModel):
    ...
    agents: list[NandaAgentFacts] = Field(..., description="List of agents")
...
class NandaAgentFactsDelta(BaseModel):
...
class NandaAgentFactsDeltaResponse(BaseModel):
    ...
    deltas: list[NandaAgentFactsDelta] = Field(..., description="List of changes")
    next_seq: int = Field(..., description="Next sequence number to query")


class NandaWellKnown(BaseModel):
    ...
    # Provider info
    provider: NandaProvider = Field(..., description="Registry provider")
    
    # Capabilities
    capabilities: list[str] = Field(
        default_factory=lambda: ["agentfacts", "deltas"],
        description="Supported NANDA capabilities"
    )
...
class NandaToolsResponse(BaseModel):
    """Response model for NANDA tools endpoint."""
    registry_id: str = Field(..., description="Registry identifier")
    tools: list[NandaTool] = Field(..., description="Available tools")


class NandaA2AMessage(BaseModel):
